...
//
// Phase 13 — the performance-correct replacement for the current GradientCard.
//
// WHY the app lags:
//   1. The existing GradientCard wraps EVERY child in Opacity(), even at 1.0.
//      Opacity forces saveLayer() — an offscreen composite — on every card,
//      every frame. With long lists this tanks scroll FPS.
//   2. 138 withOpacity() calls allocate new Color objects each build.
//
// Fixes here:
//   - Opacity is only inserted when actually dimmed (< 1.0).
//   - A precomputed, const-friendly gradient/border (no per-build withOpacity
//      when the accent is the default).
//   - Optional `repaintBoundary` so a card in a scrolling list doesn't repaint
//      siblings.
//
